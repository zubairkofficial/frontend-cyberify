...
const SearchSelect = ({ label = "", placeholder=label, value, onChange, required = false, error="", options = [], iscustom, style = {} }) => {
    return (
        <div className={`fv-row ${iscustom ? "" : "mb-3"}`}>
            {label && <label className={`fw-semibold fs-6 mb-2 ${required ? 'required' : ''}`}>{label}</label>}
            <ReactSelect styles={customStyles} placeholder={placeholder} value={value} options={options} onChange={onChange} />
            {/* <select className="form-control form-control-solid mb-3 mb-lg-0" value={value} onChange={onChange}>
