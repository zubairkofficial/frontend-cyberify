...
const BasicSelect = ({ label = "", placeholder=label, value, onChange, required = false, error="", options = [], isObject = false, optionValue = '', optionLabel = '', style = {}, iscustom = false, isSmall = false, showError = true }) => {
    return (
        <div class={`fv-row ${iscustom ? "" : "mb-3"}`}>
            {label && <label class={`fw-semibold ${isSmall ? 'fs-10' : 'fs-6'} mb-2 ${required ? 'required' : ''}`}>{label}</label>}
            <select style={style} class={`form-control form-control-solid mb-3 mb-lg-0 ${isSmall ? 'fs10' : ''}`} value={value} onChange={onChange}>
