...
const Biddings = () => {
    ...
    const getBiddings = (datetype = dateType) => {
        if(currentFileter !== 'custom_dates'){
            axios.get(`${Helper.apiUrl}bidding/bidding-stats/auth/${currentFileter}/${datetype}`, Helper.authHeaders).then(response => {
    ...
    const getBiddingsByDate = (datetype = dateType) => {
        if(dates.from_date && dates.to_date){
            setLoading(true);
            axios.get(`${Helper.apiUrl}bidding/bidding-stats/auth/${currentFileter}/${datetype = dateType}/${dates.from_date}/${dates.to_date}`, Helper.authHeaders).then(response => {
                setStats(response.data);
                setBiddings(response.data.all_jobs);
                setPaginated(Helper.paginate(response.data.all_jobs));
            }).catch(error => {
    ...
    return (
        <div class="d-flex flex-column flex-column-fluid">
            ...
            <MainContent>
                <FullRow>
                    <button className={`btn btn-switch-left ${dateType === 'created_at' && 'switch-active'}`} onClick={() => handleDateType('created_at')}>Created Date</button>
                    <button className={`btn btn-switch-right ${dateType === 'updated_at' && 'switch-active'}`} onClick={() => handleDateType('updated_at')}>Updated Date</button>
                </FullRow>
                <br />
                <Row>
                ...
                <Row>
                    ...
                    <Column className="mb-3" cols={3}>
                        <Card>
                            <p>Amount Spent ($)</p>
                            <h1>USD {stats.amount_usd}</h1>
                        </Card>
                    </Column>
                    <Column className="mb-3" cols={3}>
                        <Card>
                            <p>Amount Spend (PKR)</p>
                            <h1>PKR {stats.amount_pkr}</h1>
                        </Card>
                    </Column>
                    <Column className="mb-3" cols={3}>
                        <Card>
                ...
                <FullRow className={'mt-3'}>
                    <Card>
                        <DataTable
                            ...
                            setPageNo={setPageNo}
                            fields={['job_name', 'job_type', 'bid_price', 'upwork_profile', 'status']}
                            title="Job Applied"
                        ...
                        >
                            <table class="table align-middle table-row-dashed fs-7 gy-5">
                                ...
                                <tbody>
                                    {paginated.length > 0 && paginated[pageNo].map(bid => {
                                        return (
                                            <tr>
                                                ...
                                                <td>${calculateSpendings(bid.connects_used).total_usd} ≈ PKR {calculateSpendings(bid.connects_used).total_pkr}</td>
